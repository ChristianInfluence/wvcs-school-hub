...
function mapInvoiceFromDatabase(row) {
  ...
  return {
    ...
    familyKey: row.family_key || invoice.familyKey || "",
    studentIds: row.student_ids || invoice.studentIds || [],
    schoolYear: row.school_year || "",
    status: row.status || "Draft",
    paymentStatus: row.payment_status || invoice.paymentStatus || "Unpaid",
    paidAt: row.paid_at || invoice.paidAt || "",
    paymentHistory: row.payment_history || invoice.paymentHistory || [],
    paymentMethod: row.payment_method || invoice.paymentMethod || "",
    checkNumber: row.check_number || invoice.checkNumber || "",
    invoice: {
      ...invoice,
      paymentStatus: row.payment_status || invoice.paymentStatus || "Unpaid",
      paidAt: row.paid_at || invoice.paidAt || "",
...
function mapInvoiceToDatabase(record, updatedByEmail = "") {
  ...
  return {
    ...
    invoice_json: {
      ...
      familyKey,
      studentIds,
      paymentStatus: record.paymentStatus || invoice.paymentStatus || "Unpaid",
      paidAt: record.paidAt || invoice.paidAt || "",
      paymentHistory: record.paymentHistory || invoice.paymentHistory || [],
      paymentMethod: record.paymentMethod || invoice.paymentMethod || "",
      checkNumber: record.checkNumber || invoice.checkNumber || "",
    },
...
export function matchesFamilyRecord(record, family) {
  ...
  const recordEmail = normalizeFamilyEmail(record?.parentEmail || record?.invoice?.parentEmail || record?.submitterEmail || "");
