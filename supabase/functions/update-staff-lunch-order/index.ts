...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { menuId, orders } = await request.json();
    const cleanMenuId = String(menuId || "");
    if (!cleanMenuId) throw new Error("Missing lunch menu.");
    ...
    if (!email) throw new Error("Please sign in before editing lunch orders.");
