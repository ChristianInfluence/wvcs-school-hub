...
function reviewStatus(action: string) {
  ...
  throw new Error("Unknown off-campus lunch review action.");
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { permissionId, review } = await request.json();
    if (!permissionId) throw new Error("Missing off-campus lunch permission.");
    ...
    if (recipient) {
      await sendEmail(
        buildFosMessage({
          ...
          subject: `WVCS Off-Campus Lunch Permission ${status}`,
          title: `Off-Campus Lunch Permission ${status}`,
          body: [
            `Hello ${updated.family_name || "WVCS Family"},`,
            status === "Approved"
              ? `${updated.student_name}'s off-campus lunch permission has been approved through ${new Date(expiresAt || "").toLocaleDateString()}.`
              : status === "Needs Correction"
                ? `${updated.student_name}'s off-campus lunch permission needs a correction before it can be approved.`
                : `${updated.student_name}'s off-campus lunch permission status is ${status}.`,
    ...
    return new Response(JSON.stringify({ reviewed: true, permission: updated }), {
